...
void LCD_init()
{
	...
	IOWR_ALTERA_AVALON_PIO_DIRECTION(TFTLCD_BASE_CTRL_BASE, 0xff); //默认设置out
	IOWR_ALTERA_AVALON_PIO_DIRECTION(TFTLCD_CMD_BASE, 0xff); //默认设置out
	IOWR_ALTERA_AVALON_PIO_DIRECTION(TFTLCD_DATA_BASE , 0xff); //默认设置out
...
alt_u16 LCD_RD_DATA()
{
	IOWR_ALTERA_AVALON_PIO_DIRECTION(TFTLCD_DATA_BASE, 0x00); //data设置为输入
	IOWR_ALTERA_AVALON_PIO_DATA(TFTLCD_CMD_BASE, 0x0|0x1|0x2); //读使能 来个低电平 读数据
	delay(1000);
	alt_u16 data =  IORD_ALTERA_AVALON_PIO_DATA(TFTLCD_DATA_BASE); //读出数据
	IOWR_ALTERA_AVALON_PIO_DATA(TFTLCD_CMD_BASE, 0x4|0x1|0x2); //读使能  读数据
	delay(1000);
	return data;
}
...
void LCD_WR_REG(alt_u16 cmd)
{
	IOWR_ALTERA_AVALON_PIO_DIRECTION(TFTLCD_DATA_BASE, 0xff); //data设置为输出
	// 写RS
